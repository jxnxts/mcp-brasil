"""HTTP client for Ministério da Justiça CKAN."""
...
from .constants import MJ_CKAN_BASE


async def listar_datasets() -> list[str]:
    return await package_list(MJ_CKAN_BASE)


async def buscar_datasets(query: str, rows: int = 20) -> dict[str, Any]:
    return await package_search(MJ_CKAN_BASE, query, rows)


async def detalhe_dataset(package_id: str) -> dict[str, Any]:
    return await package_show(MJ_CKAN_BASE, package_id)
