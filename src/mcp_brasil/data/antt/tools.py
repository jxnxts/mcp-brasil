...
async def listar_datasets_antt(filtro: str | None = None, limite: int = 50) -> str:
    """Lista datasets do portal ANTT.

    Args:
        filtro: Substring para filtrar por nome.
...
async def buscar_datasets_antt(termo: str, limite: int = 20) -> str:
    """Busca datasets ANTT por palavra-chave.

    Args:
        termo: Palavra-chave (ex: 'acidente', 'pedágio', 'carga').
...
async def detalhe_dataset_antt(package_id: str) -> str:
    """Detalhe de um dataset ANTT com URLs dos recursos para download.

    Args:
        package_id: Nome do package (ex: 'acidentes-rodovias').
    """
...
async def datasets_chave_antt() -> str:
    """Datasets ANTT mais usados (curadoria, offline)."""
    rows = [(k, v) for k, v in DATASETS_CHAVE.items()]
    return "**Datasets-chave da ANTT**\n\n" + markdown_table(["package", "descrição"], rows)
